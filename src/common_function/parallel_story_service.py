...
class ParallelStoryService:
    ...
    async def generate_story_parallel(
        ...
        user_id_int: int = None,  # PostgreSQL integer user ID
        child_id_int: int = None,  # PostgreSQL integer child ID
        **kwargs
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            # Phase 5: Save to PostgreSQL + S3
            print(f"💾 Phase 5: Saving to PostgreSQL + S3...")
            sys.stdout.flush()
            logger.info("💾 Phase 5: Persisting to PostgreSQL with S3 media...")
            
            try:
                from src.story.story_persistence_service import StoryPersistenceService
                from src.db.session import AsyncSessionLocal
                
                # Use the integer IDs passed as parameters
                if user_id_int and child_id_int and AsyncSessionLocal:
                    persistence_service = StoryPersistenceService()
                    
                    async with AsyncSessionLocal() as db:
                        # Save complete story with all scenes, images, and audio to PostgreSQL + S3
                        await persistence_service.save_complete_story(
                            db=db,
                            user_id=user_id_int,
                            child_id=child_id_int,
                            story_data=manifest,
                            thumbnail_data=thumbnail_data,
                            scene_images_data=image_data,
                            scene_audio_data=audio_data
                        )
                    
                    print(f"✅ Story saved to PostgreSQL + S3!")
                    sys.stdout.flush()
                    logger.info("✅ PostgreSQL persistence complete")
                else:
                    logger.warning(f"⚠️ Skipping PostgreSQL save - missing user_id_int={user_id_int} or child_id_int={child_id_int} or db={AsyncSessionLocal is not None}")
                    print(f"⚠️ PostgreSQL save skipped (user_id_int={user_id_int}, child_id_int={child_id_int}, db_available={AsyncSessionLocal is not None})")
                    sys.stdout.flush()
                    
            except Exception as e:
                logger.error(f"❌ PostgreSQL persistence failed: {str(e)}")
                print(f"⚠️ PostgreSQL save failed (continuing with Firestore): {str(e)}")
                sys.stdout.flush()
                import traceback
                traceback.print_exc()
                # Don't fail the whole generation if PostgreSQL save fails
                # Continue to Firestore save as fallback
            
            # Save final story metadata to Firestore (legacy support)
            print(f"💾 Saving story metadata to Firestore (legacy)...")
            sys.stdout.flush()
            await self.storage_service.save_story_metadata(
                story_id, user_id, title, user_prompt, manifest
            )
            print(f"✅ Firestore metadata saved!")
            sys.stdout.flush()
